...
```{r}
names(STINTS)
STINTS_Reduced <- STINTS %>% dplyr::select(c(SeasonGame, quarter, lineups, lineups_H, lineups_A, stint_times, PM))
STINTS_Reduced
```
...
```{r}
names(df_dummys)
mod1 <- lm(PM ~ . -SeasonGame -quarter -lineups -stint_times, data=df_dummys)
summary(mod1)

mod2 <- lm(PM ~ Anna * Diego, data=df_dummys)
summary(mod2)
```
